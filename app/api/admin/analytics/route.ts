...
export async function GET() {
  try {
    ...
    // Get all published courses (admin can see all courses)
    ...
    console.log("[ADMIN_ANALYTICS] Found courses:", courses.length);

    // If no published courses, return empty analytics
    if (!courses || courses.length === 0) {
      console.log("[ADMIN_ANALYTICS] No published courses found");
    ...
    const courseAnalytics = courses.map((course) => {
      try {
        ...
        console.error(`[ADMIN_ANALYTICS] Error processing course ${course.id}:`, error);
        return {
          id: course.id,
          title: course.title,
          sales: 0,
    ...
    console.log("[ADMIN_ANALYTICS] Calculated totals:", { totalRevenue, totalSales });
